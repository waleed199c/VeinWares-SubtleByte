...
namespace ClientUI.UniverseLib.UI.ObjectPool;
...
public abstract class Pool
{
    ...
    /// </summary>
    public static ClientUI.UniverseLib.UI.ObjectPool.IPooledObject Borrow(Type type)
        => GetPool(type).DoBorrow();

    /// <summary>
    /// Borrow an object from the pool, creating a new object if none are available.
    /// </summary>
    public static T Borrow<T>() where T : ClientUI.UniverseLib.UI.ObjectPool.IPooledObject
        => (T)GetPool(typeof(T)).DoBorrow();

    /// <summary>
    /// Return the object to the pool.
    /// </summary>
    public static void Return(Type type, ClientUI.UniverseLib.UI.ObjectPool.IPooledObject obj)
        => GetPool(type).DoReturn(obj);

    /// <summary>
    /// Return the object to the pool.
    /// </summary>
    public static void Return<T>(T obj) where T : ClientUI.UniverseLib.UI.ObjectPool.IPooledObject
        => GetPool(typeof(T)).DoReturn(obj);

    protected abstract ClientUI.UniverseLib.UI.ObjectPool.IPooledObject DoBorrow();
    protected abstract void DoReturn(ClientUI.UniverseLib.UI.ObjectPool.IPooledObject obj);
}

/// <summary>
/// Handles object pooling for all <typeparamref name="T"/> objects. Each <typeparamref name="T"/> has its own <see cref="Pool{T}"/> instance.
/// </summary>
public class Pool<T> : Pool where T : ClientUI.UniverseLib.UI.ObjectPool.IPooledObject
{
    // Static
    ...
    public Pool()
    {
        ...
        InactiveHolder.transform.parent = ClientUI.UniverseLib.UI.UniversalUI.PoolHolder.transform;
    ...
    protected override ClientUI.UniverseLib.UI.ObjectPool.IPooledObject DoBorrow() 
        => BorrowObject();
    ...
    protected override void DoReturn(ClientUI.UniverseLib.UI.ObjectPool.IPooledObject obj)
        => Return((T)obj);
    ...
    public void ReturnObject(T obj)
    {
        if (!borrowed.Contains(obj))
            Plugin.Log(LogLevel.Warning,$"Returning an item to object pool ({typeof(T).Name}) but the item didn't exist in the borrowed list?");
        else
            borrowed.Remove(obj);
